  ...
  if (!sandwich) {
    ...
    $(query).each(function(index, item, done) {
      ...
      if (!found) {
        ...
        $(query).click(function() {
          $.waitForElement('h1:contains("Customize Your Order")', done)
        });
      }
    });
  }
...
var customizeOrder = function(done) {
  async.series([
    ...
    $('a:contains("Add To Order")').go(false, 'click'),
    $.go(false, 'waitForElement', 'h1:contains("Your Order For Delivery")'),
...
var checkout = function(done) {
  async.series([
    ...
    $.go('capture', __dirname + '/final.png'),
